/**
 * Legacy RideBT provider (reverse-engineered).
 *
 * This is the original `btApi` implementation, relocated behind the provider
 * abstraction unchanged. It talks to the RideBT Joomla AJAX endpoints that were
 * reverse-engineered from ridebt.org. It was the default provider until
 * 2026-07-22, when the official `bt4u` provider took over after live daytime
 * validation; this remains available via `EXPO_PUBLIC_API_PROVIDER=ridebt`.
 *
 * See API_DOCUMENTATION_LEGACY.md for the endpoint reference.
 */
...
import * as mockApi from '@/services/api/btApi.mock';
import { fetchTripsPageEmbeddedJson } from '@/services/api/routeScheduleHtml';
...
const fetchRouteTripsPageEmbeddedJson = async (routeShortName: string) => {
  if (DEBUG_USE_MOCK_API) {
...
export const ridebtProvider: TransitApiProvider = {
  id: 'ridebt',
  fetchVehicles,
  fetchRoutes,
  fetchRoutePatterns,
