...
export default function StopsScreen() {
  ...
  useEffect(() => {
    trackScreenView('stops');
  }, []);

  useEffect(() => {
    if (!selectedStop) return;

    trackEvent('stops.stop_selected', {
      stopId: selectedStop.code?.trim() || selectedStop.id,
      routesServed: selectedStop.routes.length,
    });
  }, [selectedStop]);

  useEffect(() => {
    if (!selectedStop || isArrivalsLoading || arrivalsError) return;

    trackEvent('stops.arrivals_loaded', {
      stopId: selectedStop.code?.trim() || selectedStop.id,
      count: arrivals.length,
    });
  }, [arrivals.length, arrivalsError, isArrivalsLoading, selectedStop]);
  ...
  const openRouteForStop = useCallback((routeId: string) => {
    ...
    trackEvent('stops.route_chip_opened', {
      routeId,
      stopId,
    });
    router.push({
