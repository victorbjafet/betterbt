# BetterBT API Documentation — LEGACY (Reverse-Engineered RideBT)

> **⚠️ This is the older, reverse-engineered API.** It documents the RideBT
> Joomla AJAX endpoints that were reverse-engineered from ridebt.org before the
> official Blacksburg Transit web service was known. It corresponds to the
> **`ridebt` provider**, which was the app's default until 2026-07-22 and is now
> a fallback option (`EXPO_PUBLIC_API_PROVIDER=ridebt`).
>
> For the **official** Blacksburg Transit BT4U web service — the cleaner,
> supported path, and the **current default** — see
> **[API_DOCUMENTATION.md](API_DOCUMENTATION.md)** (the `bt4u` provider).
>
> Both providers implement the same `TransitApiProvider` contract and return the
> same normalized `Bt*` types, selected by `API_PROVIDER` in `constants/config.ts`.
